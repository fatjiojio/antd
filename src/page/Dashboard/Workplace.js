      ...
      <StepsForm onFinish={async () => {
        ...
        render: ({ form, onSubmit, step, onPre }) => {
            return [
                ...
                step > 0 && (<Button key="pre" onClick={() => {
                    onPre === null || onPre === void 0 ? void 0 : onPre();
                }}>
                  上一步
                </Button>),
                <Button key="next" loading={loading} type="primary" onClick={() => {
        ...
        <StepsForm.StepForm name="base" title="创建实验" onFinish={async () => {
        setLoading(true);
        await waitTime(2000);
        setLoading(false);
        return true;
    }}>
          <ProFormText name="name" label="实验名称" width="md" tooltip="最长为 24 位，用于标定的唯一 id" placeholder="请输入名称" rules={[{ required: true }]}/>
        ...
        return (<ProFormCheckbox.Group name="checkbox" label="迁移类型" options={dbName ? ['完整 LOB', '不同步 LOB', '受限制 LOB'] : ['完整 LOB']}/>);
    }}
          </Form.Item>
        </StepsForm.StepForm>
        <StepsForm.StepForm name="time" title="发布实验">
          <ProFormCheckbox.Group name="checkbox" label="部署单元" rules={[
        {
            required: true,
        },
    ]} options={['部署单元1', '部署单元2', '部署单元3']}/>
          <ProFormSelect label="部署分组策略" name="remark" rules={[
        {
            required: true,
        },
        ...
        { value: '2', label: '策略二' },
          ...
          <ProFormSelect label="Pod 调度策略" name="remark2" initialValue="2" width="md" options={[
        {
            value: '1',
            label: '策略一',
        },
